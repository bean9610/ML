...
def backward():
    x = tf.placeholder(tf.float32, [None, mnist_forward.INPUT_NODE])   #真实值占位
    y_ = tf.placeholder(tf.float32, [None, mnist_forward.OUTPUT_NODE])
    y = mnist_forward.forward(x, REGULARIZER)  #预测值
    global_step = tf.Variable(0, trainable=False)

    ce = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=y, labels=tf.argmax(y_, 1))   #交叉熵损失函数
    ...
    train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss, global_step=global_step) #梯度下降更新学习率

    ema = tf.train.ExponentialMovingAverage(MOVING_AVERAGE_DECAY, global_step)  #滑动平均值
